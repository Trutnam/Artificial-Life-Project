...
def compute_actuation(t: ti.i32):
    """
    OPEN-LOOP ACTUATION:
    We define a simple sine wave for each actuator i, with an optional phase shift
    """
    ...
    for i in range(n_actuators):
        ...
        actuation[t, i] = amplitude * ti.sin(2 * math.pi * freq * t * dt + phase_shift)
...
def visualize(s, folder):
    ...
    for i in range(n_particles):
        color = 0x111111
        if aid[i] != -1:
            # color depends on the actuation value, purely for visualization
            act = actuation_[s, int(aid[i])]
...
def main():
    ...
    forward(steps) 
    final_loss = loss[None]
    print("Final open-loop loss (just for reference) =", final_loss)

    folder = 'open_loop_output'
    for s in range(0, steps, 32):  
        visualize(s, folder)


    plt.title("Open-Loop Simulation (No Gradient Updates)")
    plt.ylabel("Loss (e.g. -distance) [for reference]")
    plt.xlabel("Single Simulation")
    plt.plot([final_loss], 'ro')
    plt.show()
